...
if [ -n "$UNRELEASED_BODY" ]; then
  ...
  if claude -p "docs/CHANGELOG-salva.md の [Unreleased] セクションを、git の ${RANGE} のコミット内容から埋めてください。対象は Salva（apps/salva/ 配下）に関する変更だけで、LaLa本体の変更は含めないこと。ファイル冒頭の『書き方』セクション（フォーマット・カテゴリ・文体）に厳密に従うこと。ユーザー目視で気づく変更だけ書き、内部リファクタ・ドキュメント・ビルド設定の変更は書かないこと。docs/CHANGELOG-salva.md 以外のファイルを変更してはいけない。" \
...
DIRTY_AFTER_PAUSE=$(git status --porcelain | grep -v '^?? docs/plans/' | grep -v 'docs/CHANGELOG-salva\.md$' || true)
