...
## 分析パイプライン（tools/reference・taste・drumlab）の落とし穴

### feature 名が物理量を名乗っていても、中身は推定値のことがある

- `drum_shape.*_per_second` は打点を数えておらず、16分profileの perplexity（実効的な散らばり）を小節長で割った**推定値**。残響やゴーストの微小なエネルギーでも perplexity が上がるため実測とずれる（YourSong のハット: 推定6.35 対 実測3.20、タクシードライバーのキック: 推定6.39 対 実測2.17）
- **feature が耳の判定と相関しないときは、まず feature 自体が壊れていないかを実測で検算する。** 2026-08-11 の対照分析で「ドラムは境界として立たない」と結論した原因の半分がこれだった（もう半分は集団のラベルがドラム観点でなかったこと）。実測は `tools/drumlab/hatrate.py`（帯域フィルタ＋onset検出で打点を数える）

### 合成音と実曲では onset 検出の効き方が違う（合成音でゴーストは作れない）

- `librosa.onset.onset_detect` の `delta` は onset_envelope の絶対値に対する閾値で、実質「立ち上がりの鋭さ」を見ている。**実曲では小さいハットが周囲の音の余韻に埋もれて検出から落ちるが、合成音は無音の背景に鋭いノイズが置かれるだけなので −27dB まで下げても検出され続ける**（ラウドネスを実曲へ揃えても変わらない）
- 合成した検証素材を機械で検算するときはこの差を前提にする。素材が意図どおりでも実測値が設計と食い違うことがあり、素材が壊れていると誤判断しやすい
- **「はっきり鳴っているか」を合成音側で機械判定することはできない。** 生成器の制約は測定でなく velocity で担保する（`docs/design/drum-pattern.md`）

### groove.json の BPM は倍取りのことがある（hiphop のハーフタイム）

- 実質80BPM前後の曲が160と記録される。hiphop ではスネアが2拍4拍でなく「4拍目だけ」に来るハーフタイムが普通で、解析側は倍のテンポとして拾う（正例23曲では YourSong・Made my day・EXPLAIN の3曲）
- テンポ分布や実時間の量（打点/秒）を扱うときは **140以上を半分にして体感テンポへ寄せた分布も併せて見る**。補正前の正例は中央値93・平均104だが、補正後は中央値89・平均93.5、四分位範囲84–97

### 混ざった音から1打点の減衰時間は測れない（定義で桁が変わる）

- 同じ素材（実曲のドラムステム・ハット）で「最後に−30dBを上回った位置」0.279s /「最初に下回った位置」0.002s /「5ms平滑して−20dB」0.027s。**定義を変えて桁が変わるなら、その指標は測れていない**
- 1つ目は**レーンが違うのに値が一致する**（8分ハットとバックビートのスネアが両方0.279s）。窓の長さがそのまま出ていたため。素朴な実装がもっともらしい嘘をつく典型
- **孤立したワンショットなら測れる**。ただし正しい定義が逆で、孤立側は「最後に上回った位置」（混合側でこれをやると窓長が出る）。**同じ指標名でも、混ざった信号と孤立した信号で正しい定義が違う**
- 対策: 複数の定義で測って桁が変わるか見る。変わるなら目標値に載せない（`tools/drumlab/kittarget.py`）

### 帯域を切った打点検出は、そのレーン専用ではない

- スネア帯(200-2000Hz)の検出器は**キックの倍音で発火**し、キック帯(30-150Hz)の検出器は**スネアのトランジェントで発火**する
- このため「他レーンの打点と重なる候補を避ける」は成立しない。キックに適用すると「全打点がスネアと同時」で1件も残らず、スネアに適用すると**本物のバックビートが全部除外され**、残るのは−48〜−61dBの小さい事象だけになる
- **対策は避けることでなく、候補ごとに実際に切り出して「自分の帯域が最大か」を検査し、通ったものだけを候補にすること。** ヒューリスティックで選んでから検算する形だと、検出器の性質を誤解したまま静かに壊れる
- 判定は**末尾フェードの前**に行う。フェード後だと尾の長いレーンが不利になる（スネアの尾は200-2000Hzに長く残るので、末尾80msを削ると7dB落ちてハット帯が最大と判定された）
- 実装は `tools/drumlab/refkit.py`

### 聴き比べ素材の音量は RMS でなく LUFS（K特性）で揃える

- **低域に偏った素材は同じRMSでも小さく聞こえる。** ドラムの比較は素材ごとに帯域の偏りが大きいので、RMSで揃えると「片方だけ音が小さい」になって比較が成立しない（実測で踏んだ）
- 実装は `kittarget.loudness_lufs`（ITU-R BS.1770 の K特性＝高域シェルフ＋ハイパスの2段）。検算は −20dBFS の正弦で 1kHz −23.3 / 60Hz −26.7 / 8kHz −19.7 LUFS
- **サンプルを差し替えたらレーンのゲインも解き直す。** ピーク正規化＋固定ゲインではサンプルのエネルギー差でバランスが崩れる。レーン単独の帯域RMSを測って目標バランスになる値を求める（`kitcompare.solve_gains`）
