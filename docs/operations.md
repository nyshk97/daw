...
開発・運用の手順書（自分用）。プロジェクトの方針・スコープは [CLAUDE.md](../CLAUDE.md)、
実装の落とし穴は [GOTCHAS.md](../GOTCHAS.md)、動作確認手順は [VERIFY.md](../VERIFY.md) を参照。

- 内部識別子は旧名 `daw` のまま（CMakeターゲット `daw`・bundle id `local.d0ne1s.daw`・プロジェクト置き場 `~/Music/daw/`・ログ `~/Library/Logs/daw/`）。TCC・Sparkle・既存データの連続性のため意図的に変更していない
