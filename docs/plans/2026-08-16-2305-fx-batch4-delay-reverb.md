...
- [x] send を上げて Delay/Reverb が意図どおり聴こえるか（Time切替・Feedbackの伸び・Toneの暗さ・Ping-pongの左右・Pre-delayの手前感・Low Cutのモヤ変化）（2026-08-17 確認済み）
- [x] ノブ・ボタンの操作感（再生中にドラッグしてもクリックノイズが出ないか）（同上）
- [x] バウンス結果に残響の尻尾が切れずに入っているか（曲末直前の音の最初のエコーが残っているか）（同上）
