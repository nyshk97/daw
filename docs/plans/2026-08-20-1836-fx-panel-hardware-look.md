...
### 方針変更
- 2026-08-20 追加判断: Sends の小ノブは「簡素ノブ」をやめ、大ノブと同じ描き方を 30px・目盛り7本で縮小（別デザインだと家族に見えなかった）。パンは **中間案**（素材はハードウェアノブ・双極の目盛り、読み方は Logic のまま＝値をノブの中に）。パンは「量」でなく「位置」なので数値で読む方が速く、中に文字があるノブはパンだけ＝意味の違いを形で残す。しっくりこなければ `drawLogicPanKnob` 1関数で Logic 銀ノブに戻せる
