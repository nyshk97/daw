...
- 2026-08-06 完了後の追補: claude 側の API エラー（認証切れ等）は stderr でなく stream-json（stdout）に
  しか出ず、アプリのトーストに理由が届かなかった → report.sh が claude 非ゼロ終了時に RUN_LOG から
  エラーメッセージを抽出して stderr へ出すよう修正（トーストは stderr 末尾行を表示するため接続される）。
  実装中に「`$VAR` 直後の全角文字を bash 3.2 が変数名として食う」既知の罠を踏んだ（`${RUN_LOG}` で解決）
