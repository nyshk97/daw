...
### 方針変更
- 2026-08-22 本人判断: 右クリック「ピッチ補正…」の併記を「有効」「編集あり・0%」→ **`Strength 80%`（pinned があれば `・手直し N`）**
  に変更。解釈語はエディタのつまみの語と繋がらず、「編集あり・0%」は付け直し後に Strength 0% のまま残った状態の説明が
  できなかった。数字ならそのまま読める
- 2026-08-22 本人判断（モック C＋D）: 下端の無声レーンを独立した帯（16px・ピッチの行より暗い地・上辺に薄い区切り線・
  鍵盤列に「unvoiced」ラベル）にした。山がピッチの行の上に乗って同じ座標系に見えていたのが紛らわしさの原因。
  併せて `yForMidi` の基準を 0.5 ずらし、行 midiLo の下辺が canvas 下端に一致するようにした（半行はみ出してレーンを
  塗りつぶしていた。先の「m を行の中心に」の修正で基準を合わせ忘れていた）
- 2026-08-22 本人判断: ノート単位の「自動の目標に戻す」を右クリックに追加（`PitchCorrections::resetNoteToAuto`）。
  手で置いたノートをドラッグで元の段へ戻しても pinned が残り（Strength に関係なく 100% で乗る）周りと揃わない、
  という穴があった。Melodyne / Logic Flex Pitch のノート単位リセットに相当
- 2026-08-22 本人判断: 右クリックの「再解析」を**廃止**（メニューから削除。`onReanalyze`・`--pitch-action reanalyze` は
  ...
  検出器が変わることは無い」ので見送り。Reset は「すべての手直しを取り消して目標音を付け直す」に改名（「自動スナップ」は
  内部用語）
