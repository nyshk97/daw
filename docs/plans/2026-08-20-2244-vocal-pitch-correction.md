...
| クリップ全体のつまみ | **強さ（%）＋速さ（ms）の2つ**。強さ=ノート中心を目標音へ寄せる割合（既定100%。ラップはピッチの揺れが大きく中途半端だと直った感が出ない）。速さ=ノート内の動きを目標へ引き寄せる時定数（既定はゆるめでビブラート・しゃくりを残す）。**ケロケロはプリセットボタン**（速さ0ms・強さ100%） |
| ノート操作 | ①上下ドラッグで目標音を変える（半音単位・スケール外も可） ②ノート境界の分割・結合（検出誤りの救済） ③ノート単位バイパス（しゃくり・話し声区間を素に戻す） ④**横ドラッグでタイミング**（隣接区間が吸収・クリップ長不変・グリッドスナップ・⌥で解除） |
| タイミング一括クオンタイズ | **入れない** |
...
| 試聴 | **メイン再生（Space）でミックスごと聴く**。変更はデバウンス後に裏レンダー→完了時に差し替え（v20 と同じ「完了までは古い音」）。**ブロブクリックでそのノートだけ単独試聴**（`effectiveDomain(clipId)`＝「previewDomain があればそれ、無ければ activeDomain」を解決する共通関数の結果のメモリ内範囲を鳴らす専用 audition 経路。`AudioFilePreview` はファイル先頭からのデコード専用で流用不可） |
...
- [x] `Source/shared/PitchCorrection.h`: クリップが持つ編集状態
  ...
  `Note = { start: BoundaryRef, end: BoundaryRef, targetMidi, bypass }` と JSON 入出力
...
### 動作確認 [人間👨‍💻]
- [ ] 実際のラップ/サビのクリップで: 開く → 自動スナップの状態を聴く → 1音だけ上下に動かす →
  しゃくりをバイパス → ケロケロプリセット → 保存して再起動し同じ音が鳴る
- [ ] 移調（v20）を後から変えても補正が残ることを確認
- [ ] デュアルモニターで別画面にウィンドウを置き、Space / ⌘Z がメインに効くことを確認
...
- 2026-08-21 Phase 3: 鍵盤は PianoRollView から部品化せず PitchEditorView 内に簡素に描いた（48px・音名のみ）。
  部品化は両者の描画を揃えたくなった時点で行う。横ズーム・スクロールは未実装（クリップ全体をウィンドウ幅に収める。
  長いテイクで狭ければ後から足す）。「Reset」は「現在のスケール設定で自動スナップし直し」（変更プレビュー経由）。
