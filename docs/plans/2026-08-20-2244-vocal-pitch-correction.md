...
- [ ] 単一 HTML モック（scratchpad・絶対パスを返答に明記）でブロブ・元/補正後カーブ・境界の分割結合・
...
- [ ] `Source/ui/PitchEditorWindow.h`（`MixerWindow` の型）＋ `PitchEditorView.{h,cpp}`。
  右クリック →「ピッチ補正…」で開く。1枚を使い回し。キーはメインへ転送
- [ ] 描画: 左に鍵盤（`PianoRollView` の鍵盤描画を部品化して共有）・グリッド（拍/小節・
  タイムラインと同じ解像度上限 `grid-resolution.md`）・ブロブ（矩形＝ノートの目標音、中に元カーブ
  （薄）と補正後カーブ（濃））・再生ヘッド（メインと連動）・スケール外の段を薄く塗る
- [ ] 上部バー: スケール（既定=プロジェクトキー・クロマチック切替・未設定時は推定値を提案して
...
- [ ] 確定のタイミング（補正なしで開いた場合）: 開いただけでは project・dirty・undo が変わらない。
...
- [ ] **プレビューの差し込みは `Clip::activeDomain` を触らない**: `Clip::renderPending()`
  ...
  **プレビューの有効性は毎回再照合する**: undo は project 全体を置き換える（`UndoStack.h:135`）ので、
  ...
  無視される
- [ ] **プレビュー専用のレンダー経路**（通常の `RenderCache` 経路に載せない）: editor generation 付きの
...
- [ ] **確定の前提条件 = サイドカーが書けていること**。サイドカーは解析完了時（＝編集が可能になる前）に
  ...
  （テストで固定）
- [ ] **対象の識別は `Clip::id`**。分割・削除・移動・undo/redo のたびに id で引き直し、見つからなければ
  プレビューを baseline へ戻して閉じる（別クリップへ baseline を戻さない）。テスト: 未確定プレビュー中に
  対象を分割/削除/undo → 閉じても他クリップが変化しない
- [ ] ノート操作: 上下ドラッグ（半音・ドラッグ中ポップアップ）・横ドラッグ（隣接吸収・グリッド
...
- [ ] バイパス（b キー or 右クリック）。**undo は1ドラッグ1件**
- [ ] ブロブクリックで単独試聴: `shared_ptr<const AudioBuffer>`＋render範囲を受ける**専用 audition
...
- [ ] 変更 → `RenderCache::requestSync()`（デバウンス）→ 完了で差し替え。
  レンダー中はブロブを少し減光（「完了までは古い音」が見えるように）
- [ ] タイムラインのリージョンにバッジ（`+2` の隣・補正有効時のみ）。右クリック項目名に
  「（有効）」併記
- [ ] `Shortcuts.h` に追加（エディタ内のキー）。⌘? 一覧に自動で載る

### Phase 4: 仕上げ・ドキュメント [AI🤖]
- [ ] `docs/design/region-settings.md` の見送りリストとオーディオトランスポーズ節を更新
  （上記「私の側で決めた細部」の文言）
- [ ] `CLAUDE.md` 実装済み一覧に1行追加（「ボーカルのピッチ補正（クリップ単位・非破壊・独立
  ウィンドウ）」）
- [ ] `GOTCHAS.md`: 実装で踏んだ罠（PSOLA の周期マーク・時変シフトのブロック境界・
  サイドカーの整合など）を追記
- [ ] `VERIFY.md`: 確認手順（合成音での再検出・無加工ビット一致・ウィンドウ操作）を追記
- [ ] `mise run test` 全通し・警告ゼロ

### AI 側の実アプリ動作確認 [AI🤖]
着手前に `~/Library/CloudStorage/Dropbox/dotfiles/.claude/references/mac-app-verification.md` を読む。
- [ ] `mise run build` → dev 版起動（ユーザーの常用版を止めない）
- [ ] オフスクリーン描画（既存の `fx-snapshots.sh` の型）でブロブ・元/補正後カーブ・未確定表示・
  レンダー中の減光・バッジを画像で確認
- [ ] 右クリック →「ピッチ補正…」で独立ウィンドウが開く／別リージョンで中身が入れ替わる／
  Space・⌘Z がメインへ転送される、を CGEvent 合成＋`~/Library/Logs/daw/` のログで確認
- [ ] 開く→閉じるで baseline 復元（project が変化しない・dirty でない）をログで確認
- [ ] `VERIFY.md` の関係手順（v20 の移調・分割・保存再読込）を再実行
