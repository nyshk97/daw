...
- 2026-07-22 実装後 /code-review（8観点＋検証）とユーザー経由の指摘を反映（v0.2.0 リリース後）:
  - 修正: ①appcast 取得失敗時、既存 Release があればエラー停止（履歴消失防止） ②fetch-sparkle.sh を毎 configure 実行（バージョン bump が既存環境に反映されない問題） ③埋め込み前に既存 Sparkle.framework を rm -rf（ditto の上書きマージでシールが壊れる問題） ④worktree 判定を `git status --porcelain` ベースに（未追跡ファイル検出。docs/plans/ は除外）＋ CHANGELOG ポーズ後の再検査 ⑤CFBundleVersion と SUFeedURL の整合検査を Step 4 に追加 ⑥本体 repo へのタグ push を Release 作成成功後に移動 ⑦SparkleBridge::shutdown() 追加（KVO 解除・破棄後コールバック防止）＋ dead code の canCheckForUpdates() 削除 ⑧build.sh の Versions/B 直書きを readlink Current で解決 ⑨MIN_OS の silent fallback をエラー停止に
  - 見送り: メニュー再構築設計の一元化（進行中の bounce-export plan がメニューバー導入を含むため、そちらで MenuBarModel を単一所有に再設計する際に Sparkle 項目も統合する。今触ると衝突する）。**bounce-export 側への申し送り**: File メニュー導入時、Sparkle の KVO コールバック（Main.cpp の rebuildMacMainMenu）が setMacMainMenu を呼んで新メニューを空モデルで上書きするため、メニュー所有を統合すること
