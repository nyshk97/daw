...
- ガチャパネル（🎲）→ Loops タブ → カード選択 →「✨ おすすめを出す」→ 候補5行＋「候補 N本中 x〜y位」。初回は上モノ特徴量の計算で数秒かかる（2回目からキャッシュ）
- 行クリック＝試聴のトグル（▮▮表示）。「次の5本 →」でページ送り。ログ `gacha.loop_recommend ref=<名前> page=<N> total=<M>` / `gacha.loop_audition file=<wav>`
- 右端の ✓ → ダイアログ「設定して敷く」→ 1小節目に Loops トラック＋ループ（2周）・LCD がループのキー/BPM に変わる・タブ上部に ⚓ アンカー行。ログ `gacha.loop_pick loop=<相対パス> applyKeyBpm=1`
...
- 「ビートを残す」→ ⌘S → プロジェクトを開き直して Bass を振り直しても `roots=anchor` のまま（アンカーの再読込追従）。keep 時にループの WAV が `clip-NNN.wav` に実体化される
